...
function startCountdown() {
    ...
    // Ajouter styles dynamiques
    if (!document.getElementById('dynamic-countdown-style')) {
        const style = document.createElement('style');
        style.id = 'dynamic-countdown-style';
        style.textContent = `
    ...
    const timer = setInterval(() => {
        ...
        if (distance < 0) {
            ...
            countdownBox.innerHTML = '<h3 style="color: #ff1744; text-align: center; font-size: 1.5rem; text-shadow: 0 0 20px rgba(255,23,68,0.5);">🎉 Le salon a commencé!</h3>';
            return;
        }
        ...
        // Couleurs dynamiques selon proximité
        const allUnits = countdownBox.querySelectorAll('.time-unit span');
        let color, glowIntensity, animation;
        ...
        if (days > 30) {
            ...
            color = '#c62828';
            glowIntensity = '20px';
            animation = 'urgentShake 0.8s ease-in-out infinite';
        }

        allUnits.forEach(unit => {
            unit.style.color = color;
            unit.style.textShadow = `0 0 ${glowIntensity} ${color}88`;
            unit.style.transition = 'all 0.3s ease';
        });

        if (timerContainer) {
            timerContainer.style.animation = animation;
        }
    }, 1000);
}
...
function injectHiddenPitchButton() {
...
// Load visit counter to record visits when this script is present on a page
(function(){
  try {
    var s = document.createElement('script');
    s.src = 'assets/js/visit-counter.js';
    s.async = true;
    document.head.appendChild(s);
  } catch(e){ console.warn('Could not load visit-counter', e); }
})();
